...
from robot_messages.msg import obstacle,point_2d

import time


try:
    from utils_updated.path_planner.rrt import *
except ImportError:
    raise
...
class RRTStar(RRT):
    ...
    def choose_parent(self, new_node, near_inds):
        ...
        for i in near_inds:
            ...
            if t_node and self.check_collision(t_node, self.obstacle_list,self.sizeFactor):
    ...
    def search_best_goal_node(self):
        ...
        for goal_ind in goal_inds:
            ...
            if self.check_collision(t_node, self.obstacle_list,self.sizeFactor):
...
#     raise
            ...
            no_collision = self.check_collision(edge_node, self.obstacle_list,self.sizeFactor)
    ...
    def line_collision_check(self,first, second, obstacleList):
        ...
        for obs_data in obstacleList:
            d = abs(a * obs_data.x + b * obs_data.y + c) / (math.sqrt(a * a + b * b))
            if d <= self.sizeFactor*obs_data.radius:
                L1 = Line(point1 = Vector2D(x1,y1),point2 = Vector2D(x2,y2))
                L2 = Line(point1 = Vector2D(obs_data.x,obs_data.y),slope=-1/L1.slope) 
    ...
    def line_collision_check1(self,first, second, obstacleList):
        ...
        for obs_data in obstacleList:
            d = abs(a * obs_data.x + b * obs_data.y + c) / (math.sqrt(a * a + b * b))
            if d <= SIZE_FACTOR*obs_data.radius:
                L1 = Line(point1 = Vector2D(x1,y1),point2 = Vector2D(x2,y2))
                L2 = Line(point1 = Vector2D(obs_data.x,obs_data.y),slope=-1/L1.slope) 
                pointOfIntersection = L1.intersection_with_line(L2)
    ...
    def update_size_factor(self,newFactor):
        self.sizeFactor =newFactor
    ...
    def update(self,obstacleList, start, goal):
        ...
        if path is None:
            ...
            nodeList=[]
            for nodes in smoothedPath_updated:
                tempNode = point_2d()
                tempNode.x,tempNode.y = nodes[0],nodes[1]   
...
# def get_obstacle_data_for_planning(robotID,team):
#     rospy.wait_for_service('get_obstacle_list_service') 
#     try:
#         obstacle_list_server = rospy.ServiceProxy('get_obstacle_list_service', obstacle_data)
#         response = obstacle_list_server(robotID,team,True)
        
#         return response.obstacle_list
...
# def handle_path_planner_request(req):

#     obstacle_list = get_obstacle_data_for_planning(req.bot_id,req.isteamyellow)
    

#     plannedPath = rrtStarInstance.update(start=(req.start_x,req.start_y),goal=(req.goal_x,req.goal_y),obstacleList=obstacle_list)
#     return rrtStarResponse(plannedPath)

# def handle_obstacle_avoidance_request(req):

#     obstacle_list = get_obstacle_data_for_planning(req.bot_id,req.isteamyellow)

#     isObstaclePresent = rrtStarInstance.line_collision_check(first=(req.start_x,req.start_y), second=(req.goal_x,req.goal_y), obstacleList=obstacle_list)
    
#     if isObstaclePresent:
#         return shouldReplanRRTResponse(False)
#     else:
#         return shouldReplanRRTResponse(True)
...
if __name__ == '__main__':
    ...
    rospy.init_node('path_planner_service',anonymous=False)
    
    # service1=rospy.Service('path_planner_service',rrtStar,handle_path_planner_request)
    # service2=rospy.Service('dynamic_obstacle_avoidance_service',shouldReplanRRT,handle_obstacle_avoidance_request)

    rrtStarInstance = RRTStar()
